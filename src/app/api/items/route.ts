    ...
    try {
        ...
        const updatedItem = await db.item.update({
            ...
            data: {
                ...
                ...(criticalThreshold !== undefined && { criticalThreshold: Number(criticalThreshold) }),
                ...(expiryWarningDays !== undefined && { expiryWarningDays: Number(expiryWarningDays) }),
                ...(storageCondition && { storageCondition }),
                ...(registrationNumber !== undefined && { registrationNumber }),
                ...(isControlledSubstance !== undefined && { isControlledSubstance: Boolean(isControlledSubstance) }),
...
export async function DELETE(request: Request) {
    try {
        ...
        if (!id) {
            return NextResponse.json({ error: "ID item wajib dikirim" }, { status: 400 });
        }
