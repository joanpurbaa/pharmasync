...
export async function GET(request: Request) {
    ...
    if (currentUserRole === "ADMIN" || currentUserRole === "STAFF") {
        ...
        return NextResponse.json({ error: "Akses Ditolak: Role tidak dikenali" }, { status: 403 });
    }
    ...
    return NextResponse.json({
        shipments: shipments.map((s) => ({
            ...
            schedule: s.scheduledAt.toLocaleDateString("id-ID", { day: "2-digit", month: "short", year: "numeric" }),
            time: s.scheduledAt.toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit" }),
        ...
        stats: { scheduled: scheduledCount, shipping: shippingCount, doneToday, activeDrivers },
        recentActivity: recentShipments.map((s) => ({
            title: s.status === "SELESAI" ? "Pengiriman Selesai" : s.status === "DIKIRIM" ? "Driver Berangkat" : s.status === "DIBATALKAN" ? "Pengiriman Dibatalkan" : "Jadwal Diperbarui",
            desc: `${s.item.name} untuk ${s.destination.name}`,
            time: s.updatedAt.toISOString(),
            type: s.status === "SELESAI" ? "success" : s.status === "DIKIRIM" ? "shipping" : "update",
        })),
