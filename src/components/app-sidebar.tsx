...
export function AppSidebar({ ...props }: React.ComponentProps<typeof Sidebar>) {
    ...
    React.useEffect(() => {
        const fetchUserAndRole = async () => {
            try {
                ...
                // Ambil role langsung dari respons backend api/auth/me
                // Pastikan backend kamu merespons dengan field role (misal: data.role)
                if (data.role) {
    ...
    const activeNavGroups = userRole === "DRIVER"
        ? [
            {
                label: "Tugas Lapangan",
                items: [
                    { name: "Dashboard", url: "/driver", icon: <LayoutDashboard /> },
                ]
            }
        ]
        : defaultNavGroups;
    ...
    return (
        <Sidebar
            ...
            <SidebarContent className="bg-white text-slate-900">
                <NavProjects groups={activeNavGroups} />
            </SidebarContent>
