...
const BOX_DIMENSION_RATIOS = {
    widthRatio: 0.82, // Dikurangi dari 0.92 - jarak horizontal antar column
    heightRatio: 1.0, // Tetap, tidak diubah sesuai permintaan
    depthRatio: 0.20, // Dikurangi dari 0.65 - jarak vertikal antar level
};
...
function buildWarehouseItems(
    ...
    const slotPool = racks.flatMap((_, rackIndex) =>
        Array.from({ length: slotsPerRack }, (_, slotIndex) => ({
            rackIndex,
            slotIndex,
        })),
    );
...
function WarehouseScene({
    ...
    React.useLayoutEffect(() => {
        ...
        if (edgeMesh) {
            edgeMesh.count = itemRenderData.length;
        }
        ...
        itemRenderData.forEach((item, index) => {
            ...
            if (edgeMesh) {
                // Edge mesh sedikit lebih besar untuk border yang jelas
                ...
                <Html position={hoverPosition} center style={{ pointerEvents: "none" }}>
                    <div className="rounded-lg border border-slate-700 bg-slate-900/95 backdrop-blur-md px-3 py-2 shadow-lg text-slate-100 text-xs leading-tight whitespace-nowrap">
                        ...
                        <div className="flex gap-3 text-[11px]">
                            <span>
                                Stok: <span className="font-medium">{hoveredData.stock}</span>
                            </span>
                            <span>
                                ED:{" "}
                                <span className="font-medium">
                                    {new Date(hoveredData.expiryDate).toLocaleDateString("id-ID")}
                                </span>
                            </span>
                        </div>
                    </div>
    ...
    React.useEffect(() => {
        ...
        async function loadItems() {
            ...
            try {
                ...
                if (isActive) {
                    setError(
                        err instanceof Error ? err.message : "Gagal memuat data warehouse",
                    );
                    setBatches([]);
                }
            } finally {
            ...
            <div className="absolute right-4 top-4 z-40 w-[300px] rounded-2xl border border-slate-800 bg-slate-950/90 backdrop-blur-lg shadow-2xl flex flex-col max-h-[calc(100%-2rem)]">
                <div className="shrink-0 border-b border-slate-800 px-5 pt-5 pb-4">
                    ...
                    <p className="text-[11px] text-slate-400 leading-relaxed">
                        Visualisasi stok barang yang telah diterima dan divalidasi oleh admin
                        secara real-time.
                    </p>
                </div>
